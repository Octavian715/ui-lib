<template>
  <div :class="wrapperClass">
    <div class="relative">
      ...
      <div v-if="$slots.suffix || clearable && inputValue || type === 'password'" class="absolute inset-y-0 right-0 flex items-center pr-3 gap-2">
        <!-- Password Toggle -->
        <button
          v-if="type === 'password'"
          type="button"
        ...
        <!-- Clear Button -->
        <button
          v-if="clearable && inputValue && !disabled && type !== 'password'"
          type="button"
          class="text-neutral-400 hover:text-neutral-300 transition-colors duration-150 focus:outline-none focus-visible:text-primary-500"
        ...
        <!-- Custom Suffix -->
        <div v-else-if="$slots.suffix" class="text-neutral-400">
          <slot name="suffix" />
        </div>
      </div>
    </div>
